...
test("authenticated author is taken from Redis; invalid and unauthenticated posts rejected", async (t) => {
  ...
  const app = createApp({
    ...
    db: {
      query: async (sql, values) => {
        parameters = values;
        return { rows: [{ id: 1, authorId: values[2] }] };
      },
    },
  });
  ...
  assert.equal(
    (await post({ title: "", content: "y" }, `REVCC_SESSION=${token}`)).status,
    400,
  );
  const result = await post(
    { title: "hello", content: "world", authorId: 99 },
    `REVCC_SESSION=${token}`,
  );
